	...
	while(sending == 1){
		...
		if(rereceive==0){
			for(int i=0;i<5;i++){
				...
				if(frame_recv.frame_kind == 0 && frame_recv.sq_no == frame_id[frame_recv.sq_no]){
			...
			for(int i=0;i<rereceive;i++){
				...
				if(frame_recv.frame_kind == 0 && frame_recv.sq_no == frame_rereceive_id[i]){
